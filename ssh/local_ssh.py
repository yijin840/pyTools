...
class ssh_client(object):
    ...
    # 设置Java路径
    def set_java_home(self, path=""):
        if path != None and path != "":  
            self._java_home = path
        if self._java_path == "" or self._java_path == None:
                self._java_path = self.find_java_path()
                self._java_home = self._java_path[0:-12] + "bin"

    # 查找Java路径
    def find_java_path(self, cmd="bash -lc 'which java'"):
        if self._java_path != "" and self._java_path != None:
    ...
    '''
    def java_jps(self):
        print(self._java_home)
        cmd = "%s/jps" % self._java_home
        return self.execute_cmd(cmd)
    ...
    def java_jmap(self, pid):
        cmd = "%s/jmap -heap " % self._java_home + str(pid)
        return self.execute_cmd(cmd)
    ...
    def java_jstat_sync(self, pid):
        cmd = "%s/jstat -gcutil %s 5000 100" % str(self._java_home,
                                                   pid)
